...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        affine_dim = num_filters * H // 2 * W // 2
    ...
    def loss(self, X, y=None):
        ...
        x2, cache_conv_relu_pool = conv_relu_pool_forward(X, W1, b1, conv_param, pool_param)
        x2_flat = x2.reshape((x2.shape[0], -1))

        x3, cache_affine_relu = affine_relu_forward(x2_flat, W2, b2)

        scores, cache_scores = affine_forward(x3, W3, b3)
        ...
        dx2_flat, dW2, db2 = affine_relu_backward(dx3, cache_affine_relu)
        dW2 += self.reg * W2

        dx2 = dx2_flat.reshape(x2.shape)
        dX, dW1, db1 = conv_relu_pool_backward(dx2, cache_conv_relu_pool)
        dW1 += self.reg * W1
